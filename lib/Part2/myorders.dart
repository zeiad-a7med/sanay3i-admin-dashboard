...
class _MyOrdersState extends State<MyOrders> {
  ...
  getRequsts() async {
    ...
    final starCountR=FirebaseDatabase.instance.ref('data/user/${FirebaseAuth.instance.currentUser?.uid.toString()}/requestsWaiting');
    ...
    final mainRef=FirebaseDatabase.instance.ref('data/requests/');
    ...
    return Scaffold(
        ...
        body:Center(
            child:RefreshIndicator(
              ...
              child:(changeState==0||!hasData)?noDataFound:ListView.builder(
                      ...
                      child: Row(
                        ...
                        children: [
                              ...
                              child: Column(
                                children: [
                                  ...
                                  Container(
                                    height: 25,
                                    width: ScreenWidth/2,
                                    child: Text("${requests[requestsIds[i]]["state"]}",style: TextStyle(color:(requests[requestsIds[i]]["state"]=="Not Accepted Yet"||requests[requestsIds[i]]["state"]=="Rejected")?Colors.red:Colors.green,fontSize: 12,),softWrap: false, maxLines: 2, overflow: TextOverflow.fade,),
                                  ),
                                ],
                              )
                          ...
                          Container(
                            ...
                            child: Align(
                              alignment: Alignment.center,
                              child:(requests[requestsIds[i]]["state"]=="Rejected")?SizedBox(
                                  ...
                                  child:ElevatedButton(
                                      ...
                                      onPressed:() async {
                                        dynamic requestId =requestsIds[i];
                                        dynamic userId =requests[requestsIds[i]]["userId"];
                                        dynamic snayiId =requests[requestsIds[i]]["sanayeeId"];
                                        await fire.deleteChild("data/requests/${requestId}");
                                        await fire.deleteChild("data/user/${userId}/requestsWaiting/${requestId}");
                                        await fire.deleteChild("data/snai3/${snayiId}/requestsWaiting/${requestId}");
                                        setState(() {
                                          changeState=0;
                                        });
                                      },
                                      child: Text("delete",style: TextStyle(color:Colors.white,fontSize: 17,fontWeight: FontWeight.w400),)
                                  )
                              ):SizedBox(
                                  height:40, //height of button
                                  width:ScreenWidth/5 , //width of button
                                  child:ElevatedButton(
                                      ...
                                      onPressed:(requests[requestsIds[i]]["state"]=="Not Accepted Yet")?null:(){
                                        ...
                                        Navigator.push(context, MaterialPageRoute(builder: (context) => AcceptedOrder(req)));
                                      },
